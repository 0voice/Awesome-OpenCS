<h1 align="center"> Open Computer Science Courses from Universities Worldwide </h1>

Curated open courses from top universities worldwide in computer science, categorized by topics like programming, algorithms, AI, and more. Contributions welcome for new courses, link updates, or suggestions to build a high-quality CS learning resource!

[Link to Chinese README](https://github.com/0voice/Awesome-OpenCS/blob/main/README.md)

## Introduction
Awesome-OpenCS is a carefully curated open-source repository of computer science course resources, featuring free open courses from leading global universities (such as MIT, Stanford, Berkeley) and platforms like Coursera and edX.

Whether you're starting with programming basics, diving deep into data structures and algorithms, or exploring cutting-edge fields like AI, computer systems, or blockchain, you'll find courses tailored to your level:
- Handpicked classics from MIT, Stanford, and Berkeley, with bilingual coverage where available.
- Clearly categorized by topic, progressing from beginner to advanced for easy entry.
- All links verified, with unified collections of lecture notes and materials to streamline self-study.
No fees required—learn CS from top professors anytime, anywhere, from theory to practice in one comprehensive hub!

Contributions welcome to build the most complete open learning community 🌟

## Table of Contents

- [Programming Fundamentals](#programming-fundamentals)
  - [Python](#python)
  - [Java](#java)
  - [C/C++](#cc)
- [Data Structures and Algorithms](#data-structures-and-algorithms)
- [Computer Systems](#computer-systems)
  - [Operating Systems](#operating-systems)
  - [Computer Architecture](#computer-architecture)
- [Autonomous Driving](#autonomous-driving)
- [Artificial Intelligence and Machine Learning](#artificial-intelligence-and-machine-learning)
  - [Machine Learning](#machine-learning)
  - [Deep Learning](#deep-learning)
  - [Natural Language Processing (NLP)](#natural-language-processing-nlp)
  - [Computer Vision (CV)](#computer-vision-cv)
- [Databases and Big Data](#databases-and-big-data)
  - [SQL/Relational Databases](#sqlrelational-databases)
  - [Hadoop/Big Data](#hadoopsbig-data)
- [Networking and Security](#networking-and-security)
  - [Computer Networks](#computer-networks)
  - [Network Security](#network-security)
- [Software Engineering and Development](#software-engineering-and-development)
  - [Web Development](#web-development)
  - [Software Engineering](#software-engineering)
- [Computer Graphics](#computer-graphics)
- [Quantum Computing](#quantum-computing)
- [Theory of Computation](#theory-of-computation)
- [Blockchain](#blockchain)
- [Contribution Guidelines](#contribution-guidelines)
- [Resource Notes](#resource-notes)

## Programming Fundamentals

These courses cover core programming basics, including introductory topics in Python, Java, and C/C++, ideal for beginners to get started quickly.

### Python

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Introduction to Computer Science and Programming in Python | MIT | English | Beginner | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-0001-introduction-to-computer-science-and-programming-in-python-fall-2016/) |
| CS50's Introduction to Programming with Python | Harvard University | English | Beginner | [CS50 Python Course](https://cs50.harvard.edu/python/2022/) |
| Introduction to Computational Thinking and Data Science | MIT | English | Beginner | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-0002-introduction-to-computational-thinking-and-data-science-fall-2016/) |

#### Notes
- **How to Choose**: For absolute beginners, start with Harvard's CS50 Python (rich in examples) or MIT's computational thinking course to blend theory with CS fundamentals.
- **Subtopic Navigation**: Sections cover "Variables and Data Types," "Functions and Control Flow," "Modules and Libraries," etc.—follow the introductory sequence for best results.

### Java

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Introduction to Programming in Java | MIT | English | Beginner | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-092-introduction-to-programming-in-java-january-iap-2010/) |
| Java Programming and Software Engineering Fundamentals | Duke University | English | Beginner | [Coursera Course Page](https://www.coursera.org/specializations/java-programming) |
| Object-Oriented Programming with Java | University of Helsinki | English | Intermediate | [MOOC.fi Course Page](https://java-programming.mooc.fi/) |

#### Notes
- **How to Choose**: Beginners should opt for Duke's specialization; for object-oriented depth, try University of Helsinki's course.
- **Subtopic Navigation**: Key sections include "Classes and Objects," "Inheritance and Polymorphism," "Exception Handling," "Collections Framework"—progress from syntax basics to OOP design.

### C/C++

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Introduction to C and C++ | MIT | English | Beginner | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-s096-introduction-to-c-and-c-january-iap-2013/) |
| CS 106B: Programming Abstractions | Stanford University | English | Beginner | [Stanford Course Page](https://web.stanford.edu/class/cs106b/) |
| Effective Programming In C And C++ | MIT | English | Beginner | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-s096-effective-programming-in-c-and-c-january-iap-2014/) |

#### Notes
- **How to Choose**: Start with Stanford's CS106B for C++ with data structures; MIT's Effective C/C++ for efficiency tips.
- **Subtopic Navigation**: Focus on "Pointers and Memory Management," "Structs and Classes," "Templates and Generic Programming" to grasp low-level logic.

## Data Structures and Algorithms

These courses cover core data structures and algorithms, including stacks, queues, linked lists, trees, graphs, sorting, searching, and dynamic programming—essential for CS fundamentals.

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Introduction to Algorithms | MIT | English | Intermediate | [MIT OCW - Introduction to Algorithms (Spring 2020)](https://ocw.mit.edu/courses/6-006-introduction-to-algorithms-spring-2020/) |
| Algorithms, Part I & II | Princeton University | English | Intermediate | [Coursera Course Page](https://www.coursera.org/learn/algorithms-part1) |
| CS 61B: Data Structures | UC Berkeley | English | Intermediate | [Berkeley Course Page](https://sp24.datastructur.es/) |
| Data Structures and Algorithms in Python | UC San Diego | English | Intermediate | [Coursera Course Page](https://www.coursera.org/learn/data-structures-algorithms-1) |

#### Notes
- **How to Choose**: Beginners: UC Berkeley CS 61B; for in-depth analysis: MIT or Princeton.
- **Subtopic Navigation**: Modules include "Linear Structures," "Trees and Graphs," "Sorting and Searching," "Dynamic Programming and Greedy Algorithms"—study in order: basics → core algorithms → advanced.

## Computer Systems

These courses explore computer systems fundamentals, including operating systems, memory management, and architecture—for a deep dive into hardware-software interfaces.

### Operating Systems

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Computer System Engineering | MIT | English | Advanced | [MIT OCW](https://ocw.mit.edu/courses/6-033-computer-system-engineering-spring-2018/) |
| CS162: Operating Systems and Systems Programming | UC Berkeley | English | Advanced | [Berkeley](https://cs162.eecs.berkeley.edu/) |
| Introduction to Computer Systems | CMU | English | Advanced | [CMU](http://www.cs.cmu.edu/~213/) |
| CS107: Computer Organization and Systems | Stanford | English | Intermediate | [Stanford](https://web.stanford.edu/class/cs107/) |

#### Notes
- **How to Choose**: Intermediate: Stanford CS107; advanced implementation: UC Berkeley CS162; pair with "Computer Systems: A Programmer's Perspective" for CMU.
- **Subtopic Navigation**: Covers "Process Management," "Memory Allocation," "File Systems," "Concurrency"—master concepts before kernel details.

### Computer Architecture

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Computation Structures | MIT | English | Advanced | [MIT OCW](https://ocw.mit.edu/courses/6-004-computation-structures-spring-2017/) |
| CS61C: Great Ideas in Computer Architecture | UC Berkeley | English | Intermediate | [Berkeley](https://cs61c.org/) |
| Parallel Computing | Stanford | English | Advanced | [Stanford CS149 Course Page](https://gfxcourses.stanford.edu/cs149/) |

#### Notes
- **How to Choose**: Intermediate: UC Berkeley CS61C; for parallel focus: Stanford CS149.
- **Subtopic Navigation**: Covers "CPU Design," "Memory Hierarchy," "Instruction Set Architecture," "Parallel Models"—build from hardware to software layers.

## Autonomous Driving
These courses cover core autonomous driving technologies, including perception, decision-making, control, and system integration—ideal for learning algorithms and engineering practices.

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Self-Driving Cars Specialization | University of Toronto (Coursera) | English | Intermediate | [Coursera Specialization](https://www.coursera.org/specializations/self-driving-cars) |
| CS330: Deep Multi-Task and Meta Learning for Autonomous Driving | Stanford University | English | Advanced | [Stanford CS330](https://cs330.stanford.edu/) |
| Autonomous Vehicles: Planning for Urban Driving | University of Pennsylvania (edX) | English | Intermediate | [edX Course Page](https://www.edx.org/course/autonomous-vehicles-planning-for-urban-driving) |

#### Notes
- **How to Choose**: Beginners: University of Toronto specialization for full stack; advanced: Stanford CS330 for deep learning applications.
- **Subtopic Navigation**: Includes "Perception Algorithms," "Path Planning," "Vehicle Control"—jump to relevant sections as needed.

## Artificial Intelligence and Machine Learning

These courses cover AI and ML fundamentals, including machine learning, deep learning, and neural networks—for exploring cutting-edge technologies.

### Machine Learning

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Introduction to Machine Learning | MIT | English | Intermediate | [MIT OCW](https://ocw.mit.edu/courses/6-036-introduction-to-machine-learning-fall-2020/) |
| Machine Learning | Andrew Ng | English | Intermediate | [Coursera](https://www.coursera.org/learn/machine-learning) |
| Machine Learning with Python | University of Michigan | English | Intermediate | [Coursera](https://www.coursera.org/learn/python-machine-learning) |

#### Notes
- **How to Choose**: Classic intro: Andrew Ng's Coursera; Python-focused: University of Michigan.
- **Subtopic Navigation**: Modules: "Supervised Learning (Regression, SVM)," "Unsupervised Learning (Clustering, Dimensionality Reduction)," "Model Evaluation"—review math basics first.

### Deep Learning

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Deep Learning Specialization | DeepLearning.AI | English | Intermediate | [Coursera Specialization](https://www.coursera.org/specializations/deep-learning) |
| CS231n: Convolutional Neural Networks for Visual Recognition | Stanford University | English | Advanced | [Stanford](http://cs231n.stanford.edu/) |
| Introduction to Deep Learning | MIT | English | Advanced | [MIT OCW](https://ocw.mit.edu/courses/6-s191-introduction-to-deep-learning-january-iap-2020/) |

#### Notes
- **How to Choose**: Beginners: DeepLearning.AI specialization; vision deep dive: Stanford CS231n.
- **Subtopic Navigation**: "Neural Network Basics," "CNNs and RNNs," "Regularization and Optimization," "Generative Models"—pair with TensorFlow/PyTorch practice.

### Natural Language Processing (NLP)

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| CS224N: Natural Language Processing with Deep Learning | Stanford University | English | Advanced | [Stanford CS224N](https://web.stanford.edu/class/cs224n/) |
| Natural Language Processing Specialization | DeepLearning.AI (Coursera) | English | Intermediate | [Coursera Specialization](https://www.coursera.org/specializations/natural-language-processing) |
| NLP with Transformers | Hugging Face & Stanford University | English | Intermediate | [Free Interactive Course](https://huggingface.co/learn/nlp-course/chapter1/1) |
| CS124: From Languages to Information | Stanford University | English | Intermediate | [Stanford CS124](https://web.stanford.edu/class/cs124/) |

#### Notes
- **How to Choose**: Beginners: DeepLearning.AI specialization; advanced theory: Stanford CS224N; practical: Hugging Face Transformers.
- **Subtopic Navigation**: "Word Embeddings," "Transformer Models," "Machine Translation"—study sequentially.

### Computer Vision (CV)

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| CS231n: Convolutional Neural Networks for Visual Recognition | Stanford University | English | Advanced | [Stanford CS231n](http://cs231n.stanford.edu/) |
| ECE 408/CS 447: Computer Vision | University of Illinois Urbana-Champaign | English | Intermediate | [UIUC Course Page](https://courses.engr.illinois.edu/ece408/sp2024/) |
| Computer Vision Specialization | DeepLearning.AI (Coursera) | English | Intermediate | [Coursera Specialization](https://www.coursera.org/specializations/computer-vision) |

#### Notes
- **How to Choose**: Intro: DeepLearning.AI; theory and classics: UIUC ECE408/CS447; deep models: Stanford CS231n.
- **Subtopic Navigation**: "Convolutional Neural Networks," "Object Detection," "Image Segmentation"—locate specific topics easily.

## Databases and Big Data

These courses cover database and big data essentials, including SQL, relational databases, and Hadoop—for data management and processing.

### SQL/Relational Databases

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Database Systems | MIT | English | Advanced | [MIT OCW Video Gallery](https://ocw.mit.edu/courses/6-830-database-systems-fall-2010/) |
| Introduction to Databases | Stanford University | English | Intermediate | [edX - Databases: Relational Databases and SQL](https://www.edx.org/learn/relational-databases/stanford-university-databases-relational-databases-and-sql) |
| CS186: Introduction to Database Systems | UC Berkeley | English | Advanced | [Berkeley Course Page](https://cs186berkeley.net/) |

#### Notes
- **How to Choose**: SQL practice: Stanford edX; deep principles: UC Berkeley CS186.
- **Subtopic Navigation**: "SQL Queries," "Relational Algebra," "Transactions and Concurrency," "Indexing and Optimization"—start with operations, then theory.

### Hadoop/Big Data

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Big Data Analysis | UC San Diego | English | Intermediate | [Coursera - Big Data Specialization](https://www.coursera.org/specializations/big-data) |
| Introduction to Big Data | UC San Diego | English | Intermediate | [Coursera - Introduction to Big Data](https://www.coursera.org/learn/big-data-introduction) |
| Big Data Technologies | University of Washington | English | Intermediate | [Coursera - Data Science at Scale Specialization](https://www.coursera.org/specializations/data-science) |

#### Notes
- **How to Choose**: Hadoop ecosystem: UC San Diego specialization; data science apps: University of Washington.
- **Subtopic Navigation**: "HDFS and MapReduce," "Spark Programming," "Big Data Storage and Analysis"—prerequisite: Java/Python basics.

## Networking and Security

These courses cover networking and security fundamentals, including protocols and cryptography—for communication and protection principles.

### Computer Networks

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Computer Networks | MIT | English | Intermediate | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-829-computer-networks-fall-2002/) |
| CS144: Introduction to Computer Networking | Stanford University | English | Intermediate | [Stanford Online Course Page](https://cs144.stanford.edu/) |
| Computer Networking: A Top-Down Approach | University of Washington | English | Intermediate | [Coursera Course Page](https://www.coursera.org/learn/computer-networking) |

#### Notes
- **How to Choose**: Top-down approach: University of Washington; protocol implementation: Stanford CS144.
- **Subtopic Navigation**: "TCP/IP Stack," "Routing and Switching," "Congestion Control," "Application Protocols"—use labs for protocol insights.

### Network Security

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Cryptography and Cryptanalysis | MIT | English | Advanced | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-875-cryptography-and-cryptanalysis-spring-2005/) |
| Cybersecurity Fundamentals | RIT | English | Intermediate | [edX Course Page](https://www.edx.org/course/cybersecurity-fundamentals) |

#### Notes
- **How to Choose**: Basics: RIT edX; crypto depth: MIT.
- **Subtopic Navigation**: "Symmetric and Asymmetric Encryption," "Digital Signatures," "Network Attacks and Defenses," "Security Protocols"—build from crypto foundations.

## Software Engineering and Development

These courses cover software engineering and development, including web and design practices—for modern software techniques.

### Web Development

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Software Engineering for Web Applications | MIT | English | Intermediate | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-171-software-engineering-for-web-applications-fall-2003/) |
| Web Development | University of Michigan | English | Intermediate | [Coursera Specialization](https://www.coursera.org/specializations/web-design) |
| Full Stack Web Development | Johns Hopkins University | English | Intermediate | [Coursera Specialization](https://www.coursera.org/specializations/html-css-javascript-for-web-developers) |

#### Notes
- **How to Choose**: Engineering focus: MIT; full stack: Johns Hopkins.
- **Subtopic Navigation**: "HTML/CSS Basics," "JavaScript Programming," "Backend Frameworks," "Frontend-Backend Interaction"—sequence: front → back → full stack.

### Software Engineering

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Foundations of Software Engineering | MIT | English | Intermediate | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-005-software-construction-spring-2016/) |
| Software Engineering: Introduction | UBC | English | Intermediate | [edX Course Page](https://www.edx.org/course/software-engineering-introduction) |

#### Notes
- **How to Choose**: Methods: UBC edX; construction principles: MIT.
- **Subtopic Navigation**: "Requirements Analysis," "Design Patterns," "Testing and Quality Assurance," "Agile Development"—apply to projects.

## Computer Graphics

These courses cover computer graphics essentials, including rendering and image processing—for visual computing.

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Computer Graphics | MIT | English | Advanced | [MIT OCW Video Gallery](https://ocw.mit.edu/courses/6-837-computer-graphics-fall-2012/) |
| CS 184: Foundations of Computer Graphics | UC Berkeley | English | Advanced | [Berkeley Course Page](https://cs184.eecs.berkeley.edu/) |

#### Notes
- **How to Choose**: Theory: MIT or Berkeley CS184.
- **Subtopic Navigation**: Check course sites for chapters like "Lecture 2: Rendering Pipeline."

## Quantum Computing

These courses cover quantum computing basics, including algorithms and information—for frontier tech exploration.

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Quantum Computation | MIT | English | Advanced | [MIT OCW Course Page](https://ocw.mit.edu/courses/18-435j-quantum-computation-fall-2003/) |
| Quantum Information Science | MIT | English | Advanced | [MIT OCW Course Page](https://ocw.mit.edu/courses/mas-865j-quantum-information-science-spring-2006/) |

#### Notes
- **How to Choose**: Start with MIT Quantum Computation for algorithms.
- **Subtopic Navigation**: Sites list chapters like "Lecture 4: Quantum Gates."

## Theory of Computation

These courses cover computation theory, including automata, complexity, and formal languages—for CS theory depth.

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Theory of Computation | MIT | English | Advanced | [MIT OCW Video Gallery](https://ocw.mit.edu/courses/18-404j-theory-of-computation-fall-2020/) |
| Automata, Computability, and Complexity | MIT | English | Advanced | [MIT OCW Course Page](https://ocw.mit.edu/courses/6-045j-automata-computability-and-complexity-spring-2011/) |
| Theory of Computation | UC Berkeley | English | Advanced | [Berkeley Course Page](https://www2.eecs.berkeley.edu/Courses/CS170/) |

#### Notes
- **How to Choose**: Systematic: MIT Automata; complexity: Berkeley.
- **Subtopic Navigation**: Chapters like "Lecture 5: Turing Machines."

## Blockchain

These courses cover blockchain fundamentals, including cryptocurrencies and distributed ledgers—for decentralized systems.

| Course Name | Source | Language | Level | Course Link |
|-------------|--------|----------|-------|-------------|
| Blockchain and Money | MIT | English | Intermediate | [MIT OCW Video Gallery](https://ocw.mit.edu/courses/15-s12-blockchain-and-money-fall-2018/) |
| Blockchain Fundamentals | UC Berkeley | English | Intermediate | [edX Course Page](https://www.edx.org/certificates/professional-certificate/uc-berkeleyx-blockchain-fundamentals) |
| Bitcoin and Cryptocurrency Technologies | Princeton University | English | Intermediate | [Coursera Course Page](https://www.coursera.org/learn/cryptocurrency) |

#### Notes
- **How to Choose**: Accessible: Princeton; tech depth: MIT or Berkeley.
- **Subtopic Navigation**: Chapters like "Lecture 3: Smart Contracts."

## Contribution Guidelines

Welcome to contribute courses or materials to **Awesome-OpenCS**! Follow these steps:

1. Fork this repository.
2. Add courses to `README.md` (maintain table format) or materials (PDFs, code) to `resources/` subfolders.
3. Submit a Pull Request describing your additions.

Ensure courses are free, links work, and filenames are clear (e.g., `MIT_6-0001_Lecture1.pdf`).

## Resource Notes

- **Sources**: International (MIT OCW, Coursera, edX).
- **Materials**: Notes, assignments, code in [resources/](resources/), categorized by topic. Download and upload to subfolders as needed.
- **License**: This project uses [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/), free to share.
